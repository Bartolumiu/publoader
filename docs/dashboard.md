...
| **Chapters** | `chapters:read` | Every chapter this platform has published, in four archives, *On MangaDex*, *Unavailable*, *Deleted*, *Edited*, filtered by extension, language, chapter number and a search that matches the series name, the chapter title and any of the four ids. Opening one shows our row beside what MangaDex says right now, anything already queued against it, and its edit history, with the three actions: edit the metadata, replace it with an unavailable card (previewed first), or delete it. The same three run in bulk over ticked rows or over the whole filter. Three cards above the listing take the same filter and ask three different questions of it: *Reconcile with MangaDex* reads MangaDex and writes these tables; *Find the chapters MangaDex has twice* (on *On MangaDex* only, folded shut) reads MangaDex per series and queues the surplus copies for deletion, needing no extension run; *Ask the publisher what is still there* (folded shut, since it is an occasional instrument and the largest one here) reads the publisher and writes MangaDex — one series picked out of the archive, or a whole extension once the filter names one. See below. |
| **Tracked** | `tracked:read` | The series map across every extension: how many mappings each has and the most recent one, linking into the map that can be edited. There is no cross-extension endpoint, so this is an index rather than a merged table. Two cards above it map **from links**, which is the only place that works without choosing an extension first: *Map a series from its links* takes one publisher page plus one MangaDex title, working out the extension and the series id from the queue, the map and a measured rule; *Map many from links* takes a paste of up to 200 such pairs, previewed as a per-row table before any of it is applied. |
...
### Finding the chapters MangaDex has twice

On the *On MangaDex* tab, folded shut beside the reconcile card: **Scan** groups
our groups' chapters by series and reports which ones MangaDex is holding more
than once — same series, same language, same publisher link. Nothing runs an
extension and nothing asks the publisher, so it answers for a series whose source
is long gone, which is where duplicates collect. Naming an extension in the
filter scopes the scan; unfiltered it walks the whole catalogue, takes a few
minutes, and the card picks a running scan back up if you navigate away.

**Delete the duplicates** is gated on having scanned, unlike the reconcile
card's buttons, and the reason is what the two do: those move rows between our
own tables, this deletes public pages, and the dialog cannot honestly say how
many or which series without a report to read it off. The oldest copy of each
survives; every later one is queued as a `DELETE`, still cancellable from
**Queues** until core-uploader drains it. Duplicates are always hard-deleted and
never carded whatever the removal mode — a card on a duplicate leaves the
duplicate in place. Deleting is ADMIN-and-up and closed to api tokens, like
