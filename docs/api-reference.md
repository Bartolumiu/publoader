...
| Method | Path | Scope | Notes |
| --- | --- | --- | --- |
| `GET` | `/chapters/reconcile` | `chapters:read` | Where the current or last pass is up to: `{state: "idle"|"running"|"done"|"failed"}` plus `progress` while running (`detail`, `done`, and `total`, which is **null** during the group walk because MangaDex reports no total up front) and `report` once done. Cheap and meant to be polled: one `settings` row, never MangaDex. A `running` state whose progress has not moved for five minutes is reported as `failed`, so a process killed mid-pass does not lock the endpoint out forever. |
