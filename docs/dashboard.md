...
| **Taking a backup from the UI** | `GET /api/v1/admin/backup` streams `pg_dump`, but the core image deliberately ships no postgres client tools, so it answers 503 with that fact. Adding `postgresql-client` to a long-lived internet-facing service widens its reach. | The scheduled `pg_dump` in [operations.md](operations.md) → "Backup and restore", which runs inside the postgres container where the tool belongs. Add the client to the core image only if you want the button, and understand that a dump contains every password hash and the saved MangaDex session. |
| **Restoring a backup** | Restoring means stopping the services that would write during it. Nothing that can take the API down should be reachable *through* the API. | The restore procedure in [operations.md](operations.md). |
