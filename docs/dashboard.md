...
| **Chapters** | `chapters:read` | Every chapter this platform has published, in four archives, *On MangaDex*, *Unavailable*, *Deleted*, *Edited*, filtered by extension, language, chapter number and a search that matches the series name, the chapter title and any of the four ids. Opening one shows our row beside what MangaDex says right now, anything already queued against it, and its edit history, with the three actions: edit the metadata, replace it with an unavailable card (previewed first), or delete it. The same three run in bulk over ticked rows or over the whole filter. Two cards above the listing take the same filter and point in opposite directions: *Reconcile with MangaDex* reads MangaDex and writes these tables, *Ask the publisher what is still there* (folded shut, since it is an occasional instrument and the largest one here) reads the publisher and writes MangaDex — one series picked out of the archive, or a whole extension once the filter names one. See below. |
