...
| **Queues** | `runs:read` | *Tasks*: the MangaDex upload queues filtered by kind and state — and by the queued chapter itself (series/title/number/either MangaDex id, extension, language), which is the only way to find an `EDIT` or `UNAVAILABLE` row, whose dedupe key is a bare MangaDex UUID. Every row names its chapter and links the series and chapter on MangaDex. With retry, cancel, and "requeue stale leases" (which only touches leases that have already expired). *Depth*: the same queues counted by kind and state. |
