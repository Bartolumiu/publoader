...
describe.skipIf(!dbReady())("queue management endpoints", () => {
  ...
  it("filters by extension, language and a chapter substring", async () => {
    ...
    expect((await only("chapter=blue%20lock")).tasks[0].id).toBe(target.id);
    expect((await only("chapter=awakening")).tasks[0].id).toBe(target.id);
    expect((await only("chapter=aaaaaaaa")).tasks[0].id).toBe(target.id);
    expect((await only("chapter=nothing-matches-this")).total).toBe(0);
