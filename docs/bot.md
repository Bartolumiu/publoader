...
| `/reconcile [extension]` | read | `chapters:read` | How many chapters MangaDex has stopped serving or deleted that the archives do not know about. **Reports only** — applying is closed to api tokens, so recording them is `padmin chapters reconcile --apply` or the dashboard. |
