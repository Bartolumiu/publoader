...
| Method | Path | Scope | Notes |
| --- | --- | --- | --- |
| `GET` | `/chapters/reconcile` | `chapters:read` | Where the current or last pass is up to: `{state: "idle"|"running"|"done"|"failed"}` plus `progress.steps` while running and `report` once done. Each step is `{id, label, state: "pending"|"running"|"done"|"skipped"|"failed", done, total, note}` — the pass declares its whole plan up front and works down it, so a poller can render what is done, what is going and what is still to come. `total` is **null** while genuinely unknown and is never a guess. A failed pass keeps its steps, with the one that died marked `failed` and the rest `skipped`. Cheap and meant to be polled: one `settings` row, never MangaDex. A `running` state whose progress has not moved for five minutes is reported as `failed`, so a process killed mid-pass does not lock the endpoint out forever. |
| `POST` | `/chapters/reconcile` | `chapters:read`, **plus ADMIN and no api tokens when `dryRun: false`** | **Starts** a pass and answers `202` — it does not wait for it. A group walk is ~124 MangaDex requests at `MANGADEX_RATELIMIT_MS`, minutes, far longer than a request survives the proxy; poll `GET /chapters/reconcile` for progress and the report. One pass at a time: a second start joins the one in flight and answers `started: false`, which is not an error. Body `{dryRun?, extensions?, skipDeleted?, skipAdopt?, skipUnavailable?}`, `dryRun` defaulting to **true**. Archives the chapters already carrying an unavailable card (`externalUrl` set **and** `pages > 0`; an external chapter with no pages is live) and the ones MangaDex 404s. Seeds archive rows for chapters with no `uploaded_chapters` row at all: the usual case, and the reason a sweep of our own table finds nothing. Also **adopts** the live chapters MangaDex holds for our groups that no table here has a row for, into `uploaded_chapters` and (where the publisher chapter id can be recovered from the URL) `uploaded_ids`; `skipAdopt` reports them without recording any, and `skipUnavailable` does the same for the carded chapters; the three skips make the three passes independently selectable, which is how the dashboard offers one button per archive. Adoption only inserts — a chapter this platform uploaded keeps the row it has. `untrackedFound`/`adoptedRecorded`/`idsRecorded` count that pass, and each group carries `live`, `untracked`, `adopted`, `adoptedWithId` and the measured `idRule` (`null` when no id could be recovered, in which case adopted rows carry no `chapter_id`). `hiddenOnMangadex` lists uncarded chapters MangaDex will not serve; those are reported and never written. Idempotent: an already-archived id keeps its original timestamp. See docs/operations.md §"Reconcile our record of the chapters with MangaDex" |

And the duplicate lookup, which asks a different question of the same
catalogue — what is MangaDex holding **twice** — and needs no extension run to
answer it:

| Method | Path | Scope | Notes |
| --- | --- | --- | --- |
| `GET` | `/chapters/duplicates` | `chapters:read` | Where the current or last scan is up to, in exactly the envelope `GET /chapters/reconcile` uses: `{state, progress.steps, report, error}`. Its own lock and its own settings row, so a scan and a reconcile pass never queue behind each other. Cheap and meant to be polled |
| `POST` | `/chapters/duplicates` | `chapters:read`, **plus ADMIN, no api tokens, and `confirm: true` when `apply: true`** | **Starts** a scan and answers `202`. Body `{apply?, confirm?, extensions?, mangaIds?}`, `apply` defaulting to **false**. Groups the chapters our groups have on MangaDex by series and asks `findDuplicateChapters` — the same function the processor uses, so a scan and a run cannot disagree about what a duplicate is. Two chapters are the same chapter when they share a language and an `externalUrl`, or (for chapters whose pages MangaDex hosts) a language, volume and number; the **oldest** survives, chapters carrying an unavailable card are excluded, and `multi_chapters` overrides spare one chapter per declared number. `mangaIds` scopes it and also changes the fetch: scoped it asks per series, unscoped it walks the whole group once (~60 requests per 6000 chapters against one per series). With `apply` each duplicate gets a `DELETE` upload task — always a hard delete, never a card, because a card on a duplicate leaves the duplicate in place — and a chapter with a `PENDING`/`LEASED` task is left alone and counted under `blocked`. The report carries `groups`, `series` (worst first, capped at 200 with the overflow in `truncatedSeries`; the counts are never capped), `seriesScanned`, `seriesWithDuplicates`, `duplicatesFound`, `queued`, `blocked`. Nothing here reaches the publisher, so it answers for a series whose source is gone |
